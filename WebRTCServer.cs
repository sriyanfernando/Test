...
namespace WebRtc.NET.Demo
{
    ...
    using Fleck;
    using LitJson;
	using JanusSignalingIntegration;
    ...
    class WebRTCServer : IDisposable
    {
       ...
       // public readonly ConcurrentDictionary<Device, String> Device_Pair = new ConcurrentDictionary<Device, string>();

		JanusRestClient client;
       ...
       // public device dev;
        ...
        public async void InitAndAttach()
		{
			var result = await client.CreateSession();
			if (result != null && result.janus.Equals(Commands.SuccessCommand))
			{
				result = await client.AttachSession();
				if (result != null && result.janus.Equals(Commands.SuccessCommand))
				{
					//var register = { "request": "join", "room": myroom, "ptype": "publisher", "display": username };
					result = await client.RegisterRoom();
					if(result != null && result.janus.Equals(Commands.SuccessCommand))
					{
                            PublishOwnFeed(true);
                            PublishRemoteFeed(true);
                    }
				}
			}
		}
        /*
        public String[] TakeVideoDevices()
        {
        ...
        */
        private void Client_OnPollingMessageReceived(JanusJsonObject jsonObj)
		{
			try
			{
				switch (jsonObj.janus)
				{
					case Commands.CreateCommand:

						break;

					case Commands.EventCommand:
						if (jsonObj.plugindata != null && jsonObj.plugindata.data != null)
						{
							switch (jsonObj.plugindata.data.videoroom)
							{
								case Commands.JoinedCommand:
									PublishOwnFeed(true);
									break;
								case Commands.EventCommand:
                                    if (jsonObj.jsep != null)
                                    {
                                        var tpe = jsonObj.jsep["type"].ToString();
                                        var sdp = jsonObj.jsep["sdp"].ToString();
                                        session.WebRtc.OnOfferReply(tpe, sdp);
                                    }
					...
					case Commands.MediaCommand:
						...
						break;
					case Commands.SlowLinkCommand:
						Debug.Write(".");

						break;
					case Commands.HangUpCommand:
						Debug.Write(".");
