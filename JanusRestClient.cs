...
namespace JanusSignalingIntegration
{
    ...
    public class audioDevice : Device
    {
        ...
        public bool audio;
        public bool audioSend;
        public bool audioRecv;

        public audioDevice(bool aud)
        {
            audio = aud;
    ...
    public class videoDevice : Device
    {
        ...
        public bool videoSend;
        public bool videoRecv;

        public videoDevice(bool vid,string deviceid)
        {
            video = vid;
            deviceId = deviceid;
        }
...
/*
        public Media(bool aud, string deviceid, bool vid)
        {
            audio = aud;
            video = vid;
            deviceId = deviceid;
        ...
        */
/*
        public Media(bool aud, bool vid, string deviceid, string wid,string hei )
        {
            audio = aud;
            video = vid;
            deviceId = deviceid;
...
*/
   ...
   // }
    ...
    public class JanusRestClient
	{
		...
		private string RoomUserName = "Sriyan";
        ...
        public string JanusHostSessionHandleDeviceEndPoint { get { return string.Format("/{0}/{1}",JanusHostSessionHandleEndPoint,DeviceId); } }
        ...
        public async Task<JanusJsonObject> UnPublish_RegisterRoom()
        {
            JanusJsonObject retVal = null;
            try
            {
                var jsonObject = new JanusJsonObject()
                {
                    janus = Commands.MessageCommand,
                    transaction = Guid.NewGuid().ToString(),
                    body = new Dictionary<string, object> { { "request", "unpublish" } }
                };
                ...
                using (var httpClient = new HttpClient { Timeout = TimeSpan.FromSeconds(5000) })
                {
                    var content = new StringContent(JsonConvert.SerializeObject(jsonObject), Encoding.UTF8, MediaType);
                    var result = await httpClient.PostAsync(JanusHostSessionHandleEndPoint, content);
                  //  var result = await httpClient.GetAsync(string.Format("{0}{1}",JanusHostSessionEndPoint,HandleId));
                    if (result.StatusCode == System.Net.HttpStatusCode.OK)
                    {
                        var body = await result.Content.ReadAsStringAsync();
                        retVal = JsonConvert.DeserializeObject<JanusJsonObject>(body.ToString());
        ...
        public async Task<JanusJsonObject> CreateOffer(string sdp, audioDevice aud,videoDevice vid)
        {

            JanusJsonObject retVal = null;
            //Dictionary<string,object> video = new Dictionary<string,object > { { "width",resolution },{"height",resolution }  };
            ...
            try
            {
                var jsonObject = new JanusJsonObject()
                {
                    janus = Commands.MessageCommand,
                    transaction = Guid.NewGuid().ToString(),
                    body = new Dictionary<string, object> { { "request", "configure" }, { "audio", aud.audio },{"video",vid.video }, { "deviceId", vid.deviceId} },
        ...
        public async Task<JanusJsonObject> CreateAnswer(bool audioSend, bool videoSend)
        {
            JanusJsonObject retVal = null;
            try
            {
                var jsonObject = new JanusJsonObject()
                {
                    janus = Commands.MessageCommand,
                    transaction = Guid.NewGuid().ToString(),
                    body = new Dictionary<string, object> { { "request", "start" }, { "room", RoomId } },
                    jsep = new Dictionary<string, string> { { "type", "answer" } }
